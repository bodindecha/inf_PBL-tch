					...
					if ($isPBLmaster) foreach (str_split("ABCDEFGHIJKLM") as $readperm) {
						...
						if ($get_perm -> num_rows >= 1) {
							...
							while ($readperm = $get_perm -> fetch_assoc()) {
								if (date("Y-m-d H:i:s") > "2024-01-08 23:59:59" && $readperm["isHead"] <> "Y") {
									$timedout = true;
									continue;
				...
				case "paper-mark": {
					...
					if ($isPBLmaster) foreach (str_split("ABCDEFGHIJKLM") as $readperm) {
						...
						if ($get_perm -> num_rows >= 1) {
							$timedout = false;
							while ($readperm = $get_perm -> fetch_assoc()) {
								if (date("Y-m-d H:i:s") > "2024-01-18 13:30:00" && $readperm["isHead"] <> "Y") {
									$timedout = true;
									continue;
