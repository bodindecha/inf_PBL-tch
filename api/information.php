<?php
	...
	switch ($type) {
		case "group": {
			...
			$get = $db -> query("SELECT a.grade,a.room,a.lastupdate,a.type,a.mbr1,a.mbr2,a.mbr3,a.mbr4,a.mbr5,a.mbr6,a.mbr7,a.adv1,a.adv2,a.adv3,a.reward,(CASE WHEN a.reward='6P' THEN 0 ELSE a.score_poster+c.score+(CASE WHEN a.reward IS NULL THEN NULL WHEN a.reward='6W' THEN 0 WHEN a.reward IN ('5N', '0P') OR ROUND(SUM(b.total)*100/COUNT(b.cmte))/100<50 THEN 2 ELSE 3 END) END) AS score FROM PBL_group a LEFT JOIN PBL_score b ON b.code=a.code AND (SELECT allow FROM PBL_cmte WHERE cmteid=b.cmte)='Y' LEFT JOIN user_score c ON c.stdid=a.mbr1 AND c.year=a.year AND c.subj='PBL' AND c.field='oph-act' WHERE a.code='$code' GROUP BY b.code");
			if (!$get) {
				...
				successState(array(
					...
					"member" => array_values(array_filter([$read["mbr1"], $read["mbr2"], $read["mbr3"], $read["mbr4"], $read["mbr5"], $read["mbr6"], $read["mbr7"]])),
					"advisor" => array_filter(array($read["adv1"], $read["adv2"], $read["adv3"])),
					"score" => $read["reward"] == "6P" ? 0 : $read["score"]
				));
			}
		} break;
		case "person": {
